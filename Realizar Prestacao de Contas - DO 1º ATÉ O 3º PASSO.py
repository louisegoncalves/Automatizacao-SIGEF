    ...
    try:
        ...
        if baixas <= limite:
            ...
            if baixas <= limite:
                ...
                exercicio_financeiro = pyautogui.confirm(text='Escolha o Exercício Financeiro', title='Exercício Financeiro' , buttons=['2024', '2025','2026'])
            ...
            while baixas <= limite:
                ...
                preencher_unidade_gestora.fill(unidade_gestora)
                preencher_gestao = realizar_prestacao_contas.locator("#txtGestao_SIGEFPesquisa")
                preencher_gestao.fill(gestao)
                ...
                with realizar_prestacao_contas.expect_popup() as popup_info:
                    ...
                    preencher_numero_empenho.fill(numero_empenho)
                    ...
                    if exercicio_financeiro == '2026':
                        try:
                            ...
                            nome_exato_da_celula = f"2026PC{menor_numero:06d}"
                    ...
                    if exercicio_financeiro == '2024':
                        try:
                            selecionar_prestacao_contas.wait_for_load_state('networkidle', timeout=30000)
                            locator_candidatos = selecionar_prestacao_contas.get_by_role("cell", name="2024PC", exact=False)
                            todos_os_textos = locator_candidatos.all_inner_texts()
                            numeros_pc = []
                            for texto in todos_os_textos:
                                if texto.startswith("2024PC"):
                                    parte_numerica_str = texto[6:]
                                    if parte_numerica_str.isdigit():
                                        numeros_pc.append(int(parte_numerica_str))
                            if not numeros_pc:
                            ...
                            menor_numero = min(numeros_pc)
                            nome_exato_da_celula = f"2024PC{menor_numero:06d}"
                            celula_para_clicar = selecionar_prestacao_contas.get_by_role("cell", name=nome_exato_da_celula, exact=True)
                            celula_para_clicar.click()                 
                ...
                while situacao != "Baixa Regular":
                    ...
                    if situacao == 'Paga':
                        ...
                        realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000) 
                        observacao.fill("1º Passo: Entregue. Prestação de Contas de " + instrumento + ", Natureza da Despesa: " + natureza + ", em nome do Credor " + credor + ", Despesa Certificada: " + despesa_certificada + ", Nota Liquidação: " + nota_liquidacao + ", Preparação de Pagamento: " + preparacao_pagamento + ", Nota de Empenho: " + nota_empenho + ".")
                        botao_confirmar = realizar_prestacao_contas.get_by_role("button", name="Confirmar a Operação")
                    ...
                    if numero_nl == "Faltou!":
                        ...
                        if situacao == "Entregue":
                            ...
                            realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000) 
                            observacao.fill("Estorno, registro de prestação de contas de diárias de Entregue para Paga.")
                            botao_confirmar = realizar_prestacao_contas.get_by_role("button", name="Confirmar a Operação")
                            botao_confirmar.wait_for()
                            botao_confirmar.click()
                            realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000)
                        ...
                        if situacao == "Em Análise":
                                ...
                                operacao.select_option(label="Entregue")
                                realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000) 
                                observacao.fill("Estorno, registro de prestação de contas de diárias de Em Análise para Entregue.")
                                botao_confirmar = realizar_prestacao_contas.get_by_role("button", name="Confirmar a Operação")
                                botao_confirmar.wait_for()
                                botao_confirmar.click()
                                realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000)
                    ...
                    if situacao == 'Entregue':
                        ...
                        realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000) 
                        observacao.fill("2º Passo: Em Análise. Prestação de Contas de " + instrumento + ", Natureza da Despesa: " + natureza + ", em nome do Credor " + credor + ", Despesa Certificada: " + despesa_certificada + ", Nota Liquidação: " + nota_liquidacao + ", Preparação de Pagamento: " + preparacao_pagamento + ", Nota de Empenho: " + nota_empenho + ".")
                        botao_confirmar = realizar_prestacao_contas.get_by_role("button", name="Confirmar a Operação")
                        botao_confirmar.wait_for()
                        botao_confirmar.click()
                        realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000)
                    ...
                    if situacao == 'Em Análise':
                        ...
                        realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000) 
                        observacao.fill("3º Passo: Baixa Regular. Prestação de Contas de " + instrumento + ", Natureza da Despesa: " + natureza + ", em nome do Credor " + credor + ", Despesa Certificada: " + despesa_certificada + ", Nota Liquidação: " + nota_liquidacao + ", Preparação de Pagamento: " + preparacao_pagamento + ", Nota de Empenho: " + nota_empenho + ".")
                        botao_confirmar = realizar_prestacao_contas.get_by_role("button", name="Confirmar a Operação")
                        botao_confirmar.wait_for()
                        realizar_prestacao_contas.wait_for_load_state('networkidle', timeout=10000) 
                        if robo_deve_parar:
